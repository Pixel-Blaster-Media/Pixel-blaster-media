...
test("canonical source routes authenticate before prepare or acceptance and keep signed URLs out of persistence", () => {
  ...
  const worker = read("lib/integrations/autohdr/source-worker.ts");
  const workerCore = read("lib/integrations/autohdr/source-worker-core.ts");
  ...
  assert.match(workflow, /markSourceQuarantined/);
  assert.match(workflow, /status:\s*"queued"/);
  assert.doesNotMatch(workflow, /ingestAutoHDRSourceFiles/);
  assert.match(worker, /verifyCanonicalImageStream/);
  assert.match(workerCore, /reserveSourceMaster/);
  const workerRun = workerCore.slice(workerCore.indexOf("export async function runOneAutoHDRSourceFile"));
  assert.ok(workerRun.indexOf("reserveSourceMaster") < workerRun.indexOf("promoteQuarantineCreateOnly"));
  assert.ok(workerRun.indexOf("promoteQuarantineCreateOnly") < workerRun.indexOf("completeSourceFile"));

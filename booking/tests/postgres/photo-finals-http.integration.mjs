...
const response=await invoke({op:'intent',requestId,intentId:randomUUID(),sha256:createHash('sha256').update(bytes).digest('hex'),byteSize:bytes.length});
...
console.log(JSON.stringify({durableIntentBeforeCapability:true,actualJpegApprovalZipRoutes:true,staleRevisionAndDigestDenied:true,browser}));

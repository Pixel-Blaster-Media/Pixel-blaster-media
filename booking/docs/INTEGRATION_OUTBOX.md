...
The scheduler drains at most three pages of five due identities, groups only those listed job types by booking, runs at most 2 booking dispatchers concurrently, and stops starting work at one shared 45-second deadline. Re-listing advances invoice/email dependencies without dispatching unlisted types; repeated identities are not retried within the invocation. The cron route reads the canonical ISO rollout watermark only from `INTEGRATION_OUTBOX_DISPATCH_NOT_BEFORE`; callers cannot supply or widen it. The scheduler and route emit aggregate counts only—never tenant IDs, booking IDs, payloads, provider responses, recipient data, or credentials.
...
`vercel.json` targets the verified Pro plan (canonical team billing readback, September 6, 2026): outbox every five minutes, reminders every ten minutes, and Autoenhance reconciliation every ten minutes offset by three minutes. This replaces the stale Hobby/daily assumption. Bounded runs are not a delivery SLA; monitor oldest due age and dead-letter counts. See [Vercel Cron usage and pricing](https://vercel.com/docs/cron-jobs/usage-and-pricing). Git auto-deployment is disabled, including main; exact-candidate release gates must approve promotion.

### Schedule/content recovery migrations

Apply and verify exact recovery migrations `20260905100500` and `20260905100700` before deploying this reminder route (`20260905100600` belongs to invoice remediation). Never replay unrelated history. Deferred aggregate/line/property triggers retain old payloads and keys, replace only never-attempted superseded creation intents, preserve unexpired processing leases, and dead-letter obsolete attempted requests for manual reconciliation. The atomic admin aggregate must explicitly call `public.refresh_booking_effects(organization_id, booking_id)` after its final line snapshots/effect seed enqueue, within the same transaction. They do not enqueue duplicate creation notices for already-completed work or invent effects for legacy admin bookings without a seed outbox payload.

Reminders use a private per-schedule queue: rolling next-24-hour eligibility, same-day retry before appointment start, two-minute leases, five-minute backoff, eight attempts and a 23-hour safe provider window from first attempt. Accepted sends require a provider ID and exact lease settlement; only the matching schedule gets the legacy stamp. Render hashes prevent changed templates/links/senders from reusing an idempotency key. Quiet bookings keep a first-attempt-only best-effort internal push, never a realtor email. Signed links are generated transiently, not stored in the queue. Provider calls already in flight cannot be recalled by a schedule edit; retained leases and dead-letter states preserve ambiguity rather than pretending exactly-once delivery.

Inspect `booking_reminder_jobs` using service-only operational columns (`status`, `attempts`, `next_attempt_at`, `first_attempt_at`, `error_code`), not payloads. Dead letters are manual reconciliation, not an automatic new-key resend. Release verification must read back deployed cron definitions and outbox enablement/watermark; this source change alone does not prove production activation. Autoenhance cadence requires MEDIA-02/04 ambiguity-safe worker integration before release.

...
export default function ResumableDownload({endpoint,identity,packageId,packageType,label}:Props){
 ...
 return <div className="min-w-0 space-y-2 rounded-md border border-slate-200 p-3" aria-label={label}>
  ...
  <div className="flex flex-wrap gap-2">
   {working?<button className={button} onClick={()=>controller.current?.pause()}>Pause</button>:progress.state==='ready'?<button className={button} onClick={()=>void controller.current?.save()}>Save ZIP</button>:<button className={button} onClick={()=>void controller.current?.start()}>{progress.state==='idle'?'Prepare ZIP':'Resume / verify ZIP'}</button>}

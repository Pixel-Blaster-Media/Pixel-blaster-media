...
try {
  ...
  const guardMigration = fs.readFileSync(path.join(root, 'supabase/migrations/20260905100800_media_rolling_receipt_guard.sql'), 'utf8');
  ...
  await sql(`insert into autoenhance_iguide_uploads(organization_id,batch_id,booking_id,iguide_portal_id,autoenhance_image_id,filename,status,warning,iguide_asset_name,iguide_job_id) values('${id}','${id}','${id}','tour','rolling-adopted','image.jpg','pending','media:claim:adopted','accepted','job');`);
  await sql(`begin; ${guardMigration} commit;`);
  const service = createClient('http://local.invalid', 'fixture', { auth: { persistSession: false, autoRefreshToken: false }, global: { fetch: async (url, init) => {
    ...
    const predicates = [...u.searchParams].filter(([k]) => !['select','order','limit','on_conflict','columns'].includes(k)).map(([k,v]) => { assert.ok(v.startsWith('eq.')); return `${ident(k)}=${lit(v.slice(3))}`; });
    ...
    if (init.method === 'POST' && u.searchParams.has('on_conflict')) statement += ` on conflict (${u.searchParams.get('on_conflict').split(',').map(ident).join(',')}) do update set ${Object.keys(payload).map(k => `${ident(k)}=excluded.${ident(k)}`).join(',')}`;
    // INSERT RETURNING belongs after ON CONFLICT, as in PostgREST.
    if (init.method === 'POST' && u.searchParams.has('on_conflict')) statement = statement.replace(' returning * on conflict', ' on conflict') + ' returning *';
    try {
  ...
  const base = loadWorkflow(service, {}, fs.readFileSync(path.join(root, 'tests/helpers/media-base-writer.ts.txt'), 'utf8'));
  const failures = [];
  for (const phase of ['unknown', 'accepted', 'adopted']) {
    const rolling = { ...input, imageId: `rolling-${phase}` };
    const claim = phase === 'adopted' ? { token: 'media:claim:adopted' } : await workflow.claimIGuideUpload(rolling);
    await workflow.upsertIGuideUpload({ ...rolling, claimToken: claim.token, status: 'pending', ...(phase === 'accepted' ? { assetName: 'accepted', jobId: 'job' } : {}), error: 'reconcile only' });
    await sql(`alter table autoenhance_iguide_uploads disable trigger autoenhance_iguide_uploads_set_updated_at; update autoenhance_iguide_uploads set updated_at='2020-01-01',media_expected_claim=${lit(claim.token)} where autoenhance_image_id=${lit(rolling.imageId)}; alter table autoenhance_iguide_uploads enable trigger autoenhance_iguide_uploads_set_updated_at;`);
    const read = async () => (await service.from('autoenhance_iguide_uploads').select('*').eq('autoenhance_image_id', rolling.imageId).single()).data;
    const before = await read();
    assert.equal(before.media_expected_claim, null, 'per-write authorization is consumed');
    assert.equal(before.media_claim_token, claim.token);
    for (const patch of [
      { status: 'uploaded', media_expected_claim: 'stale' },
      { status: 'uploaded' },
      { media_expected_claim: claim.token, iguide_portal_id: 'other' },
      ...(phase !== 'unknown' ? [{ media_expected_claim: claim.token, iguide_asset_name: 'replacement' }] : []),
    ]) {
      const result = await service.from('autoenhance_iguide_uploads').update(patch).eq('id', before.id);
      assert.match(result.error?.message ?? '', /media receipt transition fenced/);
      assert.deepEqual(await read(), before);
    }
    let acquired = false;
    try { acquired = await base.claimIGuideUpload(rolling); } catch (error) { assert.match(error.message, /media receipt transition fenced/); }
    if (acquired) failures.push(`${phase}: literal base reclaimed protected pending receipt`);
    if (!acquired) assert.deepEqual(await read(), before, 'blocked claim preserves entire receipt');
    let completed = false;
    try { await base.upsertIGuideUpload({ ...rolling, status: 'uploaded', assetName: 'stale', jobId: 'stale', processComplete: true }); completed = true; } catch (error) { assert.match(error.message, /media receipt transition fenced/); }
    if (completed) failures.push(`${phase}: literal base stale completion overwrote protected receipt`);
    if (!completed) assert.deepEqual(await read(), before, 'blocked stale completion preserves entire receipt');
  }
  assert.deepEqual(failures, [], 'rolling media authorization boundary');
  console.log('PASS literal base acquisition and stale upsert blocked for aged unknown and accepted receipts');
  ...
  input.imageId = 'retry';
  await sql(`insert into autoenhance_iguide_uploads(organization_id,batch_id,booking_id,iguide_portal_id,autoenhance_image_id,filename,status,warning,updated_at) values('${id}','${id}','${id}','tour','retry','image.jpg','failed','media:retryable:1','2020-01-01');`);
  ...
  const beforePoll = await service.from('autoenhance_iguide_uploads').select('*').eq('autoenhance_image_id', input.imageId).single();
  ...
  const afterPoll = await service.from('autoenhance_iguide_uploads').select('*').eq('autoenhance_image_id', input.imageId).single();
  ...
  const reconciled = await reconcile.pushFinishedImagesToIGuide({ ...input, batch: { ...input.batch, iguide_portal_id: 'tour', iguide_uploaded_image_ids: [] }, images: [{ imageId: input.imageId, imageName: 'image.jpg' }] });
  assert.equal(polls, 1);
  assert.ok(reconciled.uploadedImageIds.includes(input.imageId));
  assert.ok(!reconciled.failedImageIds.includes(input.imageId));

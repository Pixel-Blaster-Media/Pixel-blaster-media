...
export default function DailyAIBriefPanel() {
  ...
  return (
    <div className="mt-4 rounded-2xl border border-brand-light/20 bg-brand/10 p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          ...
          className="rounded-full bg-brand px-4 py-2 text-xs font-semibold text-white transition hover:bg-brand-light disabled:opacity-60"
        >
      ...
      {error ? (
        <p className="mt-3 rounded-xl border border-amber-300/35 bg-amber-300/10 px-3 py-2 text-sm text-amber-100">
          {error}
        </p>
      ) : null}
      ...
      {brief ? (
        <div className="mt-4 space-y-3">
          <p className="rounded-2xl border border-white/10 bg-ink/40 p-3 text-sm leading-6 text-white">
            {brief.overview}
          </p>
              ...
              <div className="mt-3 grid gap-3 xl:grid-cols-2">
                {brief.perShoot.map((shoot) => (
                  <a
                    ...
                    className="block rounded-2xl border border-white/10 bg-white/[0.03] p-3 transition hover:border-brand-light/45"
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-ink-muted">{shoot.realtor}</p>
                      </div>
                      <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] text-ink-muted">
                        Open
                      </span>
                    </div>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-ink/40 p-3">
      ...
      {items.length > 0 ? (
        ...
        <p className="mt-2 text-sm text-ink-muted">Nothing flagged.</p>
      )}
    </div>
  );
}

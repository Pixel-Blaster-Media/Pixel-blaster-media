...
const handler=createFinalsHandler({
 async authorize(_request,bookingId){
  ...
  const {data,error}=await db.from('bookings').select('id,organization_id,property_id,owner_id,status').eq('id',bookingId).eq('organization_id',p.organizationId).maybeSingle<{id:string;organization_id:string;property_id:string;owner_id:string;status:string}>();
...
export async function POST(request:Request,context:{params:Promise<{bookingId:string}>}){return handler(request,(await context.params).bookingId);}

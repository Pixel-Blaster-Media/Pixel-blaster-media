...
ZIPs use deterministic ZIP32 STORE entries in selection order, fixed DOS epoch and generated `001.jpg`, `002.jpg`, etc. No user filenames or directory paths enter the archive. Full-res preserves source bytes; MLS entries equal the verified MLS derivative bytes. ZIPs spool into a private `mkdtemp` directory and are uploaded in bounded 8 MiB multipart parts. Only one source JPEG and its current transform are processed at a time; no whole-shoot RAM buffer. Local test object capture is intentionally in-memory and is **not** the production worker's buffering design.

Limits remain 100 files, 32 MiB/source, 1 GiB selected original bytes, 16,384 px/side and 100M pixels/source. ZIP disk usage is bounded approximately 2.2 GB; the current synchronous local job has a 15-minute storage abort budget and 30-second decoder budgets. These are **not Vercel memory/disk/runtime certification** and DB transport waits are not yet bounded by this timer.
...
- A crash/partial package failure leaves the release unavailable and its immutable objects retained. Reprocessing recreates deterministic bytes create-only; existing/ambiguous writes succeed only after matching complete GET verification. No generic deletion capability or fake cleanup success.
...
The final local candidate passed 608 repository tests, 11 focused finals tests, the new PG17 package integration suite (48 malformed-evidence rollback probes and four observed-lock races), all three existing ingest/canonical PostgreSQL suites, TypeScript, ESLint and the production build. Existing Node typeless-package and Next middleware-deprecation warnings remain. This is local executable evidence, not live storage/runtime or independent review approval.

## Scope delta and remaining gates

No ingest migration, ingest worker, ingest fixtures or storage adapter edits. The additive migration replaces the canonical job transition trigger function only to recognize the new package job lifecycle; the original ingest branch/rules are preserved. Shared type declarations and generated setup SQL include the additive schema. Existing ingest and canonical suites must remain green.
...
6. Existing Vercel Node is only a candidate. Certify maximum JPEG memory, approximately 2.2 GB temp-disk allowance, processing deadline, DB timeouts, cancellation, concurrency and authenticated scheduling. A resumable per-item checkpoint may be needed if its measured limits reject this local job budget. No new paid service is assumed.
